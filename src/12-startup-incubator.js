...
export class Incubator {
  ...
  async admitStartup(startup) {
    ...
    if (this.#startups.some((ele) => ele.startup.name === startup.name)) {
  ...
  removeStartup(name) {
    // Your code here
    const idx = this.#startups.findIndex(
      (ele) => ele.startup.name === name
    );
  ...
  async assignMentor(startupName, mentor) {
    ...
    const exists = this.#startups.some(
      (ele) => ele.startup.name === startupName
    );
  ...
  async conductDemo(startupName) {
    ...
    return {
      ...
      feedback:
        feedbackArr[Math.floor(Math.random() * feedbackArr.length)],
      timestamp: new Date().toISOString(),
    };
  }
  ...
  async batchProcess(startups) {
    ...
    return Promise.allSettled(
      startups.map((ele) => this.admitStartup(ele))
    );
  }
  ...
  getStartupsByDomain(domain) {
    ...
    return this.#startups
      .filter((ele) => ele.startup.domain === domain)
      .map((ele) => ele.startup);
  }
