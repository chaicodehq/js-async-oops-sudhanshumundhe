...
export function assignRider(order) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const randomRider = riderPool[Math.floor(Math.random() * riderPool.length)];
    resolve({ ...order, rider: randomRider, status: "assigned" });
  });
}
...
export function processMultipleOrders(orderList) {
  ...
  const deliveryPromises = orderList.map((order) =>
